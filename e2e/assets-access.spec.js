...
test('adds a friend and grants only that user shared portfolio access', async ({ browser }) => {
  ...
  await expect(sharedPrimary.getByRole('button')).toHaveCount(3)
  ...
  await friendPage.getByLabel('활동 종류').selectOption('review')
  await friendPage.getByRole('button', { name: '검색', exact: true }).click()
  await expect(friendPage.getByRole('heading', { name: '점검 이력' })).toBeVisible()
