...
2026-09-26 · 현재 구조와 새 코드 배치 원칙. 저장 모델은 ADR-0008, 공유는 ADR-0009가 기준이며 [배포 기록](./deployment.md)은 별도로 확인한다. 대규모 폴더 이동을 요구하지 않는다.
...
| src/features/assets | 자산 목록·상세·표 편집. useSpreadsheetSave가 표 저장 직렬화와 saving 상태를 소유 |
| src/features/portfolio | 자산 CRUD·시세 동기화 actions/hooks와 파생 계산. 공유·게스트 접근 액션은 auth 기능에 둔다. |
| src/features/strategy, activity, lifecycle | 태그별 배분/원칙, 활동 기록 표현, 할 일·기록 조회/편집과 기능별 데이터 처리. 별도 review/자료 화면은 없음 |
...
- 자산/배분/활동/원칙의 새 흐름은 관련 features 하위에 배치하고 기존 자산을 전면 재작성하지 않는다. 금융 계산·권한·원자성은 목적별 DB RPC에, MCP 공개 계약은 `_shared/mcp/portfolio-tools.ts`에 둔다.
