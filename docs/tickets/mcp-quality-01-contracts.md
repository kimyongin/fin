...
- [ ] schema-valid 브리핑이 실제 MCP 저장→재조회→웹 표시에서 빈 항목 없이 표시된다.
- [ ] 자산 유형별 보정의 정상/0/음수/숫자 타입/알 수 없는 필드/누락 입력을 검증한다.
- [ ] 대표 실제 응답이 output schema를 만족하고 기존 저장 데이터도 표시된다.
- [ ] 출력 형식만 맞는 것이 아니라 소유권·참조·원자성 DB 테스트도 유지한다.
