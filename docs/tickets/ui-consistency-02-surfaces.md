...
우선순위: P1 · 상태: 핵심 로컬 구현 완료 / 후속 2건 남음 · 2026-09-21
...
- [ ] 저장 성공 후 재조회 실패를 중복 저장으로 복구하지 않는다.
- [ ] 브리핑→연결 상세→목록 복귀와 resize 중 초안 유지가 동작한다. 앱 내부 연결 상세 뒤로가기는 완료했으나 브라우저 history 뒤로가기는 연결하지 않았다.
...
`ModalShell`에 고정 header/body/footer, 모바일 전체 높이, PC detail drawer, safe-area, dirty/pending 닫기 정책을 연결했다. 푸터 닫기도 shell의 동일 정책을 거치며 계좌·종목·보유·태그·뉴스 편집에 적용했다. 판단/할 일과 브리핑 상세는 detail variant를 사용한다. 구현 커밋은 `1fbddf8`, 확대 적용과 회귀 수정은 `e6623dc`다.

남은 두 항목은 UI 모양이 아니라 상태 계약이다. 기존 hash 라우팅을 깨지 않도록 상세 surface와 브라우저 history의 관계를 별도 설계해야 한다. 또한 기존 편집 action은 RPC 성공 뒤 `refreshState` 실패도 편집 오류로 잡으므로, 저장 완료와 새로고침 실패를 분리해 전역 재조회 안내로 보내야 한다.
