...
- [ ] 전체 화면에서 저장/롤백/오류/개수와 닫기를 사용할 수 있고 배경 탐색이 겹치지 않는다.
- [ ] 필터→붙여넣기/행 편집→확대→축소→저장 후 재조회가 동일 데이터로 이어진다.
- [ ] 저장 중 중복 저장/초안 유실을 방지하고 실패 시 수정값이 남는다.
- [ ] 모바일에서도 표 내부 가로 스크롤과 키보드로 입력/저장에 도달한다.
