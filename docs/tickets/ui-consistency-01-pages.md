...
- [x] 자산과 판단·할 일의 시작 위치, 탭/필터/행동 계층이 같은 규칙을 따른다.
- [x] 자산 4개 보기, 다중 계좌, 태그 필터, 목표 비교, 보유/계좌 추가 진입이 유지된다.
- [x] 필터/탭 변경과 상세 왕복에서 목록 문맥이 유지되고 빈 결과를 구분한다.
- [x] 공유 읽기에서 허용된 화면만 보이며 편집 행동이 노출되지 않는다.
...
`PageControls`의 `ViewTabs`, `FilterChips`, `PageToolbar`를 판단·할 일과 자산에 적용했다. 탭은 Arrow/Home/End 키, ARIA tab/panel 연결을 지원한다. 기능별 중복 상단 여백과 빈 도구 영역을 제거했으며 기존 자산 4개 보기와 편집 진입은 유지했다. 구현 커밋은 `7dee08d`, 전체 회귀 보완은 `e6623dc`다.
