...
### 인수 조건
- [ ] 키보드 Tab/Shift+Tab이 모달 안에 머무르고 닫은 뒤 호출 위치로 복귀한다.
- [ ] 모달에 이름과 dialog 의미가 있고 배경을 키보드로 조작할 수 없다.
- [ ] 저장 중 Escape/닫기 정책이 금융 재시도 규약과 일치한다.
- [ ] 중첩 상세와 360/390/768/1024/1440px에서 입력·안전 영역·스크롤이 유지된다.
...
### 인수 조건
- [ ] 각 오류 fixture가 명세된 코드와 복구 의미로 반환된다.
- [ ] 예상 밖 오류는 내부 내용을 사용자에게 노출하지 않으면서 로그 식별자로 조사할 수 있다.
- [ ] 로그에 인증정보·사용자 원문이 포함되지 않는다.
- [ ] initialize/tools/list/tools/call 및 인증·입력·실패 계약의 재현 가능한 로컬 검사가 있다.
