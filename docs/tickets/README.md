...
## 설계 완료·구현 대기 — 원칙 마크다운 단순화 (#118)

추가 확정: 변경 메모를 선택 입력으로 저장하고 타임라인에는 날짜·메모를 표시한다. 항목 선택 시 해당 시점 본문 전체를 읽으며 기존 본문 쌍 펼침을 대체한다. 같은 원칙 이력 행을 사용한다.

[#118 원칙의 분류·적용 범위를 제거하고 자유 마크다운으로 단순화](https://github.com/kimyongin/fin/issues/118) · [로컬 명세](./ui-11-markdown-principles.md). 사용자 입력은 본문 하나로 줄이고 DB/RPC/MCP/가이드의 분류·범위 의존을 함께 제거한다. 기존 원칙 이력과 #114 타임라인은 유지한다. #114의 분류·범위 표시/기존 쓰기 계약 유지 요구만 대체하며 아직 구현하지 않았다.
