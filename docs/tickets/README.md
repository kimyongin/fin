...
| #35 저장·조회 | 로컬 핵심 구현 | 브리핑+출처+조사 범위, 판단+질문, 실행 계획·부분 체결, 상태 전이, idempotency | 보유 이유/task 관계, 정정 근거와 전체 사용자 흐름 검증 |
| #36 오늘 화면 | 부분 완료 | 소유자용 오늘·판단·할 일·원칙·보유 이유 UI | 지인 온보딩, 전체 반응형/빈 상태/왕복 감소 검증 |
...
| #38 보정·취소 | 부분 구현 | 타입별 보정, 확인 범위, 거래 취소와 task 진행도 재계산, 기준점 전후 재생, 모바일 UI | 기존 표 편집의 명시적 기준점 통합 |
...
| #43 개인 기준 | 로컬 핵심 구현 | 선택 필드, 부분 patch, version/history, MCP, 원칙 UI | 전략 변경 이력/당시 참조와 기능별 공유 통합 |
