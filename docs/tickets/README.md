...
2026-09-21 운영 관점의 정적 코드 리뷰를 작업 명세로 정리했다. **#49~#53은 티켓 작성만 완료했으며 구현은 시작하지 않았다.** 동시 실행·응답 유실 등의 위험은 각 티켓의 재현 테스트로 먼저 확인한다. 이번 리뷰에서 해당 장애를 실환경 재현하거나 자동 테스트를 실행한 것은 아니다.

| 우선순위 | GitHub 티켓 | 로컬 명세 |
| --- | --- | --- |
| P1 | [#49 매매 동시 처리·저장 재시도](https://github.com/kimyongin/fin/issues/49) | [명세](./operations-01-financial-writes.md) |
| P1 | [#50 사용자 전환·비동기 상태 격리](https://github.com/kimyongin/fin/issues/50) | [명세](./operations-02-async-state.md) |
| P1 | [#51 검증·서버 호환성·배포 순서](https://github.com/kimyongin/fin/issues/51) | [명세](./operations-03-release-gates.md) |
| P2 | [#52 대상별 조회·공유 응답 필드](https://github.com/kimyongin/fin/issues/52) | [명세](./operations-04-scoped-reads.md) |
| P2 | [#53 공통 모달·MCP 오류 진단](https://github.com/kimyongin/fin/issues/53) | [명세](./operations-05-ui-errors.md) |

권장 구현 순서는 #49 → #50 → #51 → #52 → #53이다. #51의 배포 게이트 설계는 앞선 수정과 함께 검토하되, 실제 운영 배포는 별도 승인한다. #53은 모달과 오류 진단을 각각 검증·커밋한다. 기능별 테이블·RPC를 유지하고 필요한 작은 공통 처리만 추출한다. 범용 CRUD/명령 프레임워크나 전면 재작성은 범위가 아니다.
