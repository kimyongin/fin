...
[#146](https://github.com/kimyongin/fin/issues/146) · [명세·승인 시안·구현 기록](./ui-34-sharing-form-and-viewers.md). 이름/비밀번호/토글을 입력 영역에 모아 한 번에 저장한다. 두 카드의 버튼은 입력 아래 독립 행 오른쪽, 안내는 별도 줄로 맞춘다. 기존 친구 관계에 마지막 웹 열람 시각 하나를 추가해 공유받는 친구 목록을 제공한다. 일반 로컬 구현·검증을 마쳤으며 운영 적용·실제 친구 계정 확인은 남았다.

[#145](https://github.com/kimyongin/fin/issues/145) · [구현 기록](./ui-33-sharing-settings-autosave.md) · [ADR-0009](../adr/0009-whole-portfolio-sharing.md). 전체 읽기 공유·개별 정책 퇴역은 일반 로컬에 구현/적용했고 운영 전이다. #146은 즉시 저장 UX를 대체하며 전체 공유 권한과 기존 공유 OFF 전환 계약은 유지한다.

## 로컬 구현·운영 전 — 태그 간격과 반복 목록 행동 (#143/#144)

- [#143](https://github.com/kimyongin/fin/issues/143) · [태그 선택 간격](./ui-32-stable-tag-selection-spacing.md): 라벨 아래 8px, 선택 개수 제거, 칩 끝의 선택 해제로 선택 전후 레이아웃 이동을 없앤다. 자산·활동 선택기와 활동 필터를 대조한다.
- [#144](https://github.com/kimyongin/fin/issues/144) · [반복 목록 행동](./activity-19-recurring-list-actions.md): 이번 완료와 중단을 목록에서 구분하고 상세의 반복 종료를 제거한다. 완료/예정 반복도 목록에서 중단 가능하게 하며 에이전트 실행 대상은 유지한다.
...
[#142](https://github.com/kimyongin/fin/issues/142) · [구현 명세](./ui-31-activity-filter-fields.md). 승인 시안에 따라 검색/기록 기간/태그를 같은 라벨·필드로 정렬하고 기간 설명·중복 날짜 라벨을 제거한다. #140의 필드 스타일을 재사용하며 기존 검색 의미는 유지한다. #141의 모달 태그 이름 검색 제거와는 별개다.

## 로컬 구현·운영 전 — 모달 필드 통일·활동 태그 검색 입력 제거 (#140/#141)

- [#140](https://github.com/kimyongin/fin/issues/140) · [모달의 라벨·입력·읽기 전용 필드 통일](./ui-29-modal-field-consistency.md): 승인 이미지, 공통 48px 필드와 잠금 표시, 전체 활성 모달 적용표. 자산 시세 쓰기 금지는 유지하고 일반 텍스트 표현만 대체한다.
- [#141](https://github.com/kimyongin/fin/issues/141) · [활동 태그 검색 입력 제거](./ui-30-activity-tags-without-search.md): 활동 추가/상세/관리창에서 전체 칩을 바로 선택한다. 활동 목록 태그 필터·참조 검색·자산 태그 관리 검색은 유지한다.
...
[#139](https://github.com/kimyongin/fin/issues/139) · [구현 명세](./ui-28-instrument-registration-and-quote-display.md). 종목만 등록→0보유 목록→상세 보유 입력, 저장하지 않는 조회, 0보유 가격 갱신 대상과 시세 읽기 전용을 로컬 구현했다. DB 567/단위 102/E2E 76개 통과 기록이 있다. 실제 Yahoo 0보유 갱신·실기기/운영 배포는 미완료다. #124의 수동 가격 및 #113/#138의 상단 보유 추가 계약을 대체한다. 후속 모달 필드 통일은 시세의 표시 방식만 바꾼다.

## 설계 완료·구현 대기 — 상단 카드 실제 구조 통일 (#138)

[#138](https://github.com/kimyongin/fin/issues/138) · [구현 티켓](./ui-27-page-panel-structure.md) · [공통 가이드](../design/page-panel-guidelines.md). 네 화면의 첫 행 제목/행동, 보조 정보, 선택적 수치, 구분선/본문을 공통 컴포넌트로 고정한다. #136의 외형 공통화만으로 남은 불일치를 보완하며 같은 폭의 네 화면 비교 캡처를 완료 조건으로 둔다. 문서만 작성했고 앱 구현은 아직이다.

## 로컬 구현·검증 — 하단 프로필·메뉴 통합 (#137)

[#137](https://github.com/kimyongin/fin/issues/137) · [헤더 제거와 하단 프로필·메뉴 통합](./ui-26-footer-profile-navigation.md). 왼쪽 아이콘 전용 프로필 전환, 가운데 네 탭, 오른쪽 메뉴로 통합했다. 이름은 선택 목록에서 표시하고 동물·과일 아이콘은 기존 내 프로필 설정에서 고른다. 상단 카드는 각 탭의 문맥·기능을 맡으며 공유 읽기 표시·현재 탭 유지·초안 보호를 포함한다. 로컬 구현과 격리 검증을 마쳤고 운영 배포는 하지 않았다.

## 설계 완료·구현 대기 — 상단 카드와 자산 태그 필터 (#136)

[#136](https://github.com/kimyongin/fin/issues/136) · [상단 카드 구조 통일과 자산 태그·미분류 필터](./ui-25-page-panels-and-asset-filters.md) · [공통 가이드](../design/page-panel-guidelines.md). 자산/배분/활동/원칙의 상단 문맥·행동·필터 배치를 통일하고, 미분류 조회 → 상세 태그 지정 → 필터 유지 복귀를 연결한다. 기존 API·권한·배분 분모를 유지한다. 문서만 작성했으며 Sol 구현 대기다.

## 설계 완료·구현 대기 — 전 영역 타이포그래피 통일 (#135)

[#135 데이터 역할별 타이포그래피 기준과 전 영역 적용](https://github.com/kimyongin/fin/issues/135) · [구현 명세](./ui-24-typography-consistency.md) · [공통 가이드](../design/typography-guidelines.md). 제목/이름·주요 값/본문/라벨·조작/보조 정보의 크기·굵기·줄간격을 통일한다. Sol은 자산+공통 역할 → 배분/활동/원칙 → 설정·인증 등 전 영역 순으로 적용·검증한다. 문서 작성 완료이며 코드 적용 전이다.

## 로컬 구현·검증 완료, 운영 전 — 보유 확인 상태 퇴역

[보유 확인 상태 퇴역](./storage-simplification-07-verification-retirement.md): 자산의 미확인 배너와 확인 전용 저장·API·MCP 도구를 제거하고, 현재값 편집·절대 보정·금융 쓰기 안전장치는 유지한다. 일반 로컬 DB와 격리 회귀는 통과했으며 운영 적용은 별도다.

## 로컬 구현·자동 검증 완료, 운영 전 — 자산 목록 3열 정돈 (#134)

[#134 종목·보유·평가 3열 헤더와 목록 정보 위계 정돈](https://github.com/kimyongin/fin/issues/134) · [명세·검증](./ui-23-asset-list-columns.md). 모바일·PC에서 이름/코드·태그, 수량/평균가·현재가, 평가금액/가격 수익률을 같은 3열로 표시한다. #129의 계산을 재사용하며 시세 기준일은 상세, 누락·오래된 상태는 목록에서도 확인한다. 로컬 자동 검증을 마쳤고 운영 배포·실기기 검증은 별개다.
...
| GitHub | 로컬 명세 | 목표 |
| --- | --- | --- |
| [#129](https://github.com/kimyongin/fin/issues/129) | [자산 목록](./ui-21-asset-list-price-context.md) | 평균가·현재가·가격 수익률, 중복 비중 제거, 태그 보조 텍스트 |
| [#130](https://github.com/kimyongin/fin/issues/130) | [배분 목록](./ui-22-allocation-list-hierarchy.md) | 태그명 제목, 변경 시에만 저장 액션 |
| [#131](https://github.com/kimyongin/fin/issues/131) | [활동 필터](./activity-17-visible-date-filters.md) | 상시 필터·실제 최근 30일 조회·미완료 할 일 유지 |
| [#132](https://github.com/kimyongin/fin/issues/132) | [단일 원칙](./principles-01-single-current-document.md) | 사용자별 최신 본문 하나와 변경 이력, 웹/MCP 통일 |
| [#133](https://github.com/kimyongin/fin/issues/133) | [에이전트 반복](./activity-18-agent-due-recurrence.md) | 매일/요일·시간, 요청 시 실행 대상 판단·결과/완료 |
...
[#128 태그 칩 디자인 통일과 관리창 사용성 개선](https://github.com/kimyongin/fin/issues/128) · [명세와 검증](./ui-20-tag-chip-guidelines.md) · [공통 가이드](../design/tag-guidelines.md). 타원형 태그를 가로 배치·줄바꿈하고 관리창은 목록→새 태그→선택 후 편집으로 정리했다. 배분 박스 안에 관리 버튼을 배치하고 자산·활동의 표시/선택/필터에 적용했다. #127의 저장·권한·관리 위치는 유지한다. 실기기와 운영 배포는 별개다.

## 로컬 구현·자동 검증, 운영 전 — 태그 관리 통일 (#127)

[#127 2차 모달 용도 제한과 배분·활동 태그 관리 통일](https://github.com/kimyongin/fin/issues/127) · [명세·검증](./ui-19-tag-management-consistency.md). 2차 모달은 의사 확인 전용으로 제한하고, 두 페이지의 태그 관리를 한 관리창 내 직접 편집으로 통일했다. 활동 상세에는 태그 선택만 남겼다. #126의 확인 계층을 재사용하며 #125의 태그 사전 편집 위치를 대체한다. 운영 배포와 미검증 상호작용은 티켓에 남긴다.

## 로컬 구현·자동 검증 완료, 운영 전 — 모달 계층 통일 (#126)

[#126 모달 계층·확인 단계 통일](https://github.com/kimyongin/fin/issues/126) · [명세와 검증](./ui-18-modal-hierarchy.md). 페이지→대상 편집→짧은 확인의 최대 2단계 모달 규칙을 적용하고, 미저장 닫기·삭제·반복/원칙 종료·보유값 저장의 확인을 통일했다. 로컬 자동 회귀는 통과했으며 남은 실패/긴 이름·실기기 검증과 운영 배포는 티켓에 명시했다.

## 로컬 주요 경로 구현·구 참조 퇴역 대기 — 기록 입력 간소화 (#125)

[#125 기록 입력 간소화·종목 연결·검색형 할 일 선택](https://github.com/kimyongin/fin/issues/125) · [명세](./activity-16-record-editor-simplification.md). 계좌별 보유 연결을 종목 연결로 바꾸고 종목/할 일은 입력 검색에서 바로 선택한다. 태그는 접기·중복 표시 없이 한 곳에서 편집하며 기록 추가/상세에 함께 적용한다. #122/#123의 충돌하는 연결·태그 UI 요구를 대체하고 #124 자동 기록과 연계한다.

## 로컬 주요 경로 구현·구 메모 퇴역 대기 — 자산 상세 간소화 (#124)

[#124 상세 입력 간소화·현재값 직접 수정·저장 사유 확인](https://github.com/kimyongin/fin/issues/124) · [명세](./ui-17-asset-detail-simplification.md). 종목 일반 메모 하나와 계좌별 보유값·현재가/기준일을 접기 없이 표시한다. 상세의 매매/증권사 확인 진입을 없애고 보유값 저장 직전에만 작은 선택적 사유 확인창을 제공한다. #120/#123의 충돌하는 자산 세부 규칙을 대체하며 원자 저장은 유지한다.

## 로컬 주요 경로 구현·전수 검증 대기 — 모든 모달 직접 편집 (#123)

[#123 모든 상세 모달에 직접 편집·단일 저장 규칙 적용](https://github.com/kimyongin/fin/issues/123) · [명세](./ui-16-modal-inline-edit.md). 모달 가이드의 공통 규칙으로 자산/활동/할 일/원칙 등 전체 진입 경로를 점검한다. #118/#120/#122와 저장 계약을 공유하고 #117의 별도 편집 모드·태그 별도 저장 유지 요구를 대체한다.

## 로컬 웹·DB·MCP 구현·운영 적용 대기 — 활동 종류 제거 (#122)

[#122 내부 종류 제거·태그/대상 참조 통합과 활동 공유 단순화](https://github.com/kimyongin/fin/issues/122) · [명세](./activity-15-tags-and-references.md). 활동은 본문·태그·할 일/보유 참조 중심이며 종류별 공유/편집 제한을 제거한다. 실제 금융/반복 상태는 실행 기능이 관리한다. #119/#120의 관련 기록 표현과 계약은 이 설계를 따른다.

## 로컬 구현·검증 중 — 태그별 배분 단순화 (#121)

[#121 모드·버킷 제거와 태그별 목표 비중 직접 편집](https://github.com/kimyongin/fin/issues/121) · [명세](./ui-14-tag-allocation.md). 현재/목표/차이를 한 목록으로 합치고 목표 합계 100%를 직접 설정한다. 별도 설정·적립금/한도 추천과 구 저장 구조를 정리하며 기존 목표의 이관/재설정·공유·MCP 계약을 포함한다.

## 로컬 구현·운영 적용 대기 — 자산 상세 직접 편집 (#120)

[#120 상세에서 속성 직접 편집·통합 저장하고 모달 왕복 제거](https://github.com/kimyongin/fin/issues/120) · [명세](./ui-13-asset-detail-inline-edit.md). 종목 공통 정보·계좌별 보유·메모를 상세에서 편집하고 원자 저장한다. 매매/실제 확인은 같은 shell의 별도 단계로 유지한다. #118/#119와 독립이며 구현 첫 단계에서 기존 RPC 기반 최소 저장 계약을 확정한다.

## 로컬 구현·관련 자동 검증 — 타임라인 디자인 통일 (#119)

[#119 활동·원칙 타임라인 디자인 가이드와 공통 표현 적용](https://github.com/kimyongin/fin/issues/119) · [명세](./ui-12-timeline-consistency.md) · [디자인 가이드](../design/timeline-guidelines.md). 날짜별 카드·시각/점·대표 텍스트·상세 진입·더 보기/상태를 통일한다. #118의 원칙 변경 메모 계약을 선행 또는 함께 구현하며 데이터 모델 통합은 하지 않는다.
...
[#118 원칙의 분류·적용 범위를 제거하고 자유 마크다운으로 단순화](https://github.com/kimyongin/fin/issues/118) · [로컬 명세](./ui-11-markdown-principles.md). 사용자 입력은 본문과 선택적 변경 메모로 줄이고 DB/RPC/MCP/가이드의 분류·범위 의존을 제거했다. 기존 원칙 이력은 유지한다. 운영 DB/ChatGPT 전환과 수동 UI 검증은 남아 있다.
...
| 티켓 | 로컬 명세 |
| --- | --- |
| [#111 오늘·자산·활동·원칙의 상단 구조 통일](https://github.com/kimyongin/fin/issues/111) | [명세](./ui-04-primary-page-top.md) |
