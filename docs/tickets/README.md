...
| #35 저장·조회 | 로컬 핵심 구현 | 브리핑+출처+조사 범위, 판단+질문, 상태 전이, idempotency | 보유 이유/task 관계, 정정 근거와 전체 사용자 흐름 검증 |
| #36 오늘 화면 | 부분 완료 | 소유자용 오늘·판단·할 일·원칙·보유 이유 UI | 지인 온보딩, 전체 반응형/빈 상태/왕복 감소 검증 |
| #37 매매 기록 | 로컬 핵심 구현 | preview/log/list MCP, numeric 원가 풀, stale/idempotency, 모바일 UI | 전량 매도·재매수 fixture와 실행 task 부분 체결 연결 |
| #38 보정·취소 | 부분 구현 | 타입별 보정, 확인 범위, 거래 취소 preview/confirm, 기준점 전후 재생, 모바일 UI | 실행 task 진행도와 기존 표 편집의 명시적 기준점 통합 |
