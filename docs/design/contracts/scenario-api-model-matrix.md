...
| S02 | 장기 투자하고 자주 매매하지 않는 것을 내 기준으로 저장 | get/save_investment_policy | profile revision/head | 목표 비중/상세 전략, 기존 브리핑 기준 | #43 |
| S03 | 카테고리 목표·적립금·모드 변경 | get/save_investment_policy(strategy patch) | 현재 strategies + strategy revision, 안정 버킷 키 | 개인 성향, 과거 결정/체결 | #43 |
| S04 | 이 종목은 장기 적립용. 실적 뒤 다시 확인 | save_holding_thesis(follow_up_tasks) | thesis revision+research task/history 원자 저장 | 기존 메모 덮어쓰기, 자동 adopted 판단/매매 | #41/#35 |
| S05 | 오늘 점검해줘 | get_daily_context → ChatGPT 외부 조사 | 업무 데이터 변경 없음 | run/열람/실제확인 생성 | #34 |
| S06 | 오늘 점검하고 저장해줘 | get_daily_context → ChatGPT 조사 → save_daily_briefing | run/scopes/근거/briefing, 선택 proposed/조사task | 원칙 수정/채택/체결 | #34/#35 |
| S07 | 새 소식 없고 확인 결과 추가 행동 불필요 | 문맥 조회 → 확인 출처 포함 save_daily_briefing | 충분 scope+checked source+no_action 브리핑 | 가짜 기사·억지 할 일 생성 | #35 |
| S08 | 일부 종목 검색 실패 | save_daily_briefing(partial) | 성공/실패 scope 분리, partial run | 실패 종목 조사 완료, 전체 no_action | #34/#35 |
| S09 | 며칠 쉬었다가 복귀 | get_daily_context, 필요 get_research_history | 조회만; 저장 요청하면 새 run | 마지막 저장일로 모든 대상 조사 기준 갱신 | #34/#44 |
| S10 | 유지하고 다음 실적 때 다시 보자. 기록해줘 | record_investment_decision(follow_up_tasks) | adopted 유지 revision + research task 원자 생성 | 실행 계획/체결 생성, 이유 자동 수정 | #35 |
...
| S12 | 정정 기사로 예전 판단 다시 검토 | get_research_history → save_daily_briefing(events/evidence/task reopen) | 새 근거/사건 revision, 재개 history | 옛 기사/판단 덮어쓰기, 자동 대체 채택 | #35/#44 |
| S13 | 제안은 봤지만 채택 안 함 / 거절 | 조회만 또는 transition_investment_decision(dismiss) | 무변경 또는 dismissed revision | adopted 결정, 실행 task/거래 | #35 |
...
| 질문 수정 후 과거 브리핑의 연결 내용이 달라질 수 있음 | briefing_tasks에 task ID뿐 아니라 당시 history/version 고정 | 현재상태와 당시내용을 구분하는 DTO |
...
미통과: 전체 JSON Schema와 샘플 자동 검증, context receipt 및 pagination 기술 검증, decimal scale/반올림·이관 데이터 확인, 물리 DDL/RLS·동시성 테스트. 사용자에게 새로운 큰 제품 선택을 요구할 사안은 현재 발견하지 않았지만 이 기술 게이트 전 구현 준비 완료로 선언하지 않는다.
