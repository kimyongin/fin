...
save_daily_briefing의 후속 계약에 proposed_decisions[], research_task_creates[], research_task_changes[]를 추가한다. batch-local key를 써서 새 근거/제안/질문을 같은 저장에서 참조하고 응답은 key→ID를 반환한다. local key는 전역 중복 판정 키가 아니다. 기존 질문 갱신에는 기존 ID+expected_version이 필요하다.

이 API에서는 adopted 결정, execution 계획, 개인 원칙 변경, 매매를 금지한다. ‘점검하고 저장’은 새 조사 질문과 근거 있는 조사 답을 저장할 수 있지만 사용자 투자 의사결정을 대신하지 않는다.

질문 한 건이라도 해당 질문/실제 필요한 의존값 버전이 충돌하면 전체 batch는 version_conflict로 저장하지 않는다. 클라이언트는 실패 내용을 숨기지 않고 새 key로 변경 없는 분석만 보존하거나 재조회·재분석한다. 분석 당시 시각/변경 경고와 실제 수정 충돌은 구분한다. 가격이나 무관한 메모 갱신만으로 질문 답 저장을 거부하지 않는다. 저장된 분석과 실제 적용한 변경 목록이 어긋나지 않게 한다.

정정 자료는 이전 근거를 참조하는 새 evidence를 추가하고 영향 받는 판단/질문 ID를 조회한다. get_research_history(subject,cursor)로 저장된 사건/근거를 찾는다. 이는 인터넷 검색 API가 아니다. 관련 판단은 재검토 대상으로 보여줄 뿐 자동 채택·보유 이유 변경은 하지 않는다.
...
물리 스키마·완전한 요청/응답 JSON Schema·명시적 공개 필드 테스트·서버 임시 context 생명주기 검증은 여전히 남아 있다. 이 문서는 시나리오를 표현할 서비스 경계를 완성한 것이지 운영 준비 완료 선언이 아니다.
