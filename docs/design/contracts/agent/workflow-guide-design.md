...
| reconciliation | 실제 현재값으로 잔고 맞추기 / 잘못된 기록의 현재값 보정. 이미 맞으면 쓰지 않는다. | find_holdings, preview_holding_reconciliation, reconcile_holding / W06 |
...
도출한 초안은 사용자가 확인한 뒤 저장한다. 이미 확정된 문구에 대해 명확히 저장을 요청했다면 같은 승인을 다시 요구하지 않는다. 분석 요청만 있으면 저장하지 않는다. 일부 수정은 해당 원칙 ID의 최신 행만 바꾸고 다른 원칙을 유지한다. 종료는 해당 원칙의 `end=true`로 처리한다. 투자 기준 저장은 별도 수치 계약인 태그별 목표 비중이나 보유 이유를 함께 수정하지 않는다.
