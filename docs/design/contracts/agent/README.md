...
2026-09-21 · 운영에는 OAuth MCP 0.4.0과 35개 도구가 배포되어 있다. 로컬 0.6.0에는 `get_workflow_guide`와 제품 피드백 2개를 더한 38개 도구와 일곱 topic을 구현했다. 자동 계약 검증과 운영 배포·ChatGPT 웹/모바일 평가는 별개이며 새 기능은 아직 운영에서 사용할 수 없다.

## 원본과 전달 경로

2026-09-21 후속 구현: 투자 기준 인터뷰를 실제 소비 사례로 [작업 가이드 제공·최신화 설계](./workflow-guide-design.md)와 #63~#65를 구현했다. 이어 #66~#68의 제품 피드백을 추가해 런타임 단일 원본, 일곱 topic, 도구 의존성·source digest 검사와 평가 사례를 연결했다. 운영 배포와 실제 모델 평가는 남아 있다.
