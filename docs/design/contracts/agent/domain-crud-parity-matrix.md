...
| 종목 C/R/U/D, 메모·대표 태그 | `app_create_instrument`, `app_get_portfolio_state`, `app_save_asset_detail_with_activity`, `app_delete_instrument` | `create_instrument`, `get_portfolio_state`, `save_asset_detail`, `delete_instrument`, `update_entity_note` | `asset_detail_current_test`, `domain_save_activity_tags_test`, `test-mcp-contract`; 가격 직접 쓰기 금지. 직접 웹 변경과 MCP 교차 조회는 미검증. |
| 계좌별 보유 C/R/U/D | `app_save_asset_detail_with_activity`, `app_get_portfolio_state`, `app_delete_holding_checked`; 매매·보정 전용 RPC | `save_asset_detail`, `get_portfolio_state`, `delete_holding`; 매매·보정 전용 도구 | `checked_holding_delete_test`, `domain_save_activity_tags_test`, `test-mcp-contract`; 시장/평가/현금·복수 계좌 MCP 저장→HTTP 조회, 버전·재시도 검증. HTTP 저장→MCP 재조회 전체 필드는 미검증. |
| 자산 태그 C/R/U/D·연결 | `app_save_tag`, `app_get_portfolio_state`, `app_delete_tag`, `app_save_asset_detail_with_activity` | `save_asset_tag`, `get_portfolio_state`, `delete_asset_tag`, `save_asset_detail` | `clear_allocation_targets_test`, `test-mcp-contract`; 목표가 남은 태그 삭제 거부. 모바일/데스크톱 태그 조작 검증은 남음. |
| 목표 배분 C/R/U/D(전체 초기화) | `app_save_allocation_targets_with_activity`, `app_get_strategy_state`, `app_clear_allocation_targets_with_activity` | `save_allocation_targets`, `get_strategy_state` | `clear_allocation_targets_test`, `domain_save_activity_tags_test`, `test-mcp-contract`; 빈 목록은 미설정으로 복귀. 화면 조작 검증은 남음. |
...
| 원칙 현재 문서·이력 C/R/U/D | `app_save_principle_with_activity`, `app_list_principles`, `app_list_principle_changes`, `app_get_principle_row`, `app_correct_principle_row`, `app_delete_principle_row` | `save_principle`, `list_principles`, `list_principle_changes`, `get_principle_row`, `correct_principle_row`, `delete_principle_row` | `principle_history_crud_test`, `domain_save_activity_tags_test`, `test-mcp-contract`; 이력행 삭제/정정은 정책 변경과 구별. |
