...
- #118~#123 구현의 미적용 15개 migration(`20260923173731`~`20260924148000`)을 일반 로컬 DB에 `supabase migration up --local`로 증분 적용했다. 사전에 데이터 덤프를 `%LOCALAPPDATA%\Temp\fin-local-before-20260924.sql`에 저장했다. 이 개인 복구 파일은 Git에 포함하거나 공유하지 않는다. 일반 로컬 `db reset`과 운영 DB 변경은 하지 않았다.
- 적용 전후 계좌 4·보유 10·활동 63행을 확인했고 일반 로컬 `npm run test:db` 42파일·539검사가 통과했다. 기존 활동 관련 공유 허용은 새 활동 단일 공유 범위로 자동 승계하지 않고 migration에서 해제한다. 사용자가 공유 범위를 확인하고 다시 켜야 한다.
- 이전 단락의 당시 최신 migration 번호와 검사 수치는 2026-09-23 시점 기록이다. 운영 DB/Edge/Pages/ChatGPT 웹·모바일에는 이 로컬 적용 결과가 반영되지 않았다.
