create or replace function public.app_save_nonmarket_holding(
    ...
    input_purchase_amount numeric,
    input_valuation_amount numeric,
    input_note text,
    input_source text,
    input_request text
)
...
begin
    ...
    if input_kind = 'cash' and (input_quantity is null or input_quantity < 0) then raise exception 'Balance must be zero or greater'; end if;
    ...
    select to_jsonb(h) into before_row from public.holdings h
    where h.user_id = current_user_id and ((input_holding_id is not null and h.id = input_holding_id) or (input_holding_id is null and h.account_id = input_account_id and h.ticker = normalized_ticker)) limit 1;
    ...
    if input_holding_id is not null then
      update public.holdings set account_id = input_account_id, ticker = normalized_ticker,
        quantity = case when input_kind = 'cash' then input_quantity else null end,
        avg_price = null,
        purchase_amount = case when input_kind = 'valuation' then input_purchase_amount else null end,
        valuation_amount = case when input_kind = 'valuation' then input_valuation_amount else null end,
      ...
      values (current_user_id, input_account_id, normalized_ticker,
        case when input_kind = 'cash' then input_quantity else null end, null,
        case when input_kind = 'valuation' then input_purchase_amount else null end,
        case when input_kind = 'valuation' then input_valuation_amount else null end,
        nullif(trim(coalesce(input_note, '')), ''))
      on conflict on constraint holdings_account_id_ticker_key do update set
        quantity = excluded.quantity, avg_price = null, purchase_amount = excluded.purchase_amount,
    ...
    values (current_user_id, input_source, case when before_row is null then 'create_holding' else 'update_holding' end,
      nullif(trim(coalesce(input_request, '')), ''), 'holdings', saved_holding_id::text, before_row, after_row, 'succeeded')
    ...
    from public.holdings h join public.accounts a on a.id = h.account_id join public.instruments i on i.user_id = h.user_id and i.ticker = h.ticker
...
create or replace function public.app_save_valuation_holding(
    ...
    input_purchase_amount numeric default null, input_valuation_amount numeric default null, input_note text default null,
...
returns table (holding_id bigint, account_id bigint, account_name text, ticker text, display_name text, quantity numeric, avg_price numeric, purchase_amount numeric, valuation_amount numeric, note text, activity_id bigint)
language sql security definer set search_path = public as $$
  select * from public.app_save_nonmarket_holding(input_holding_id, input_account_id, input_ticker, 'cash', input_balance, null, null, input_note, input_source, input_request);
$$;

grant execute on function public.app_save_valuation_holding(bigint, bigint, text, numeric, numeric, text, text, text) to authenticated;
grant execute on function public.app_save_cash_holding(bigint, bigint, text, numeric, text, text, text) to authenticated;
...
language plpgsql security definer set search_path = public as $$
declare
  current_user_id uuid := auth.uid(); input_row jsonb; type_value text; ticker_value text; account_value text; name_value text;
  account_id_value bigint; instrument_id_value bigint; tag_id_value bigint; holding_id_value bigint;
  before_rows jsonb := '[]'::jsonb; after_rows jsonb := '[]'::jsonb; prior_holding jsonb;
...
begin
  ...
  for input_row in select value from jsonb_array_elements(input_rows) loop
    account_value := trim(coalesce(input_row ->> 'account_name', '')); name_value := trim(coalesce(input_row ->> 'display_name', ''));
    type_value := lower(trim(coalesce(input_row ->> 'instrument_type', 'market'))); ticker_value := upper(trim(coalesce(input_row ->> 'ticker', '')));
    quantity_value := nullif(input_row ->> 'quantity', '')::numeric; average_value := nullif(input_row ->> 'avg_price', '')::numeric;
    purchase_value := nullif(input_row ->> 'purchase_amount', '')::numeric; valuation_value := nullif(input_row ->> 'valuation_amount', '')::numeric;
    ...
    if type_value = 'cash' and (quantity_value is null or quantity_value < 0) then raise exception 'Cash requires a nonnegative balance'; end if;
    ...
    if account_id_value is null then insert into public.accounts (user_id, name, broker, is_active) values (current_user_id, account_value, nullif(trim(coalesce(input_row ->> 'broker', '')), ''), true) returning id into account_id_value; created_accounts := created_accounts + 1;
    else update public.accounts set broker = nullif(trim(coalesce(input_row ->> 'broker', '')), '') where id = account_id_value; end if;
    ...
    select to_jsonb(h) into prior_holding from public.holdings h where h.user_id = current_user_id and h.account_id = account_id_value and h.ticker = ticker_value;
    if prior_holding is not null then before_rows := before_rows || jsonb_build_array(prior_holding); end if;
    insert into public.holdings (user_id, account_id, ticker, quantity, avg_price, purchase_amount, valuation_amount, note)
    values (current_user_id, account_id_value, ticker_value,
      case when type_value in ('market', 'cash') then quantity_value else null end,
      ...
      case when type_value = 'valuation' then valuation_value else null end,
    ...
    select to_jsonb(h) into prior_holding from public.holdings h where h.id = holding_id_value;
    after_rows := after_rows || jsonb_build_array(prior_holding); saved_holdings := saved_holdings + 1;
  end loop;
  insert into public.activity_events (user_id, source, action_type, target_table, before_data, after_data, status)
  values (current_user_id, 'user', 'bulk_edit_portfolio', 'portfolio', jsonb_build_object('holdings', before_rows), jsonb_build_object('holdings', after_rows, 'row_count', saved_holdings), 'succeeded') returning id into event_id;
