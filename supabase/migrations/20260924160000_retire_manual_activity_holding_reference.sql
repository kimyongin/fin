-- Manual records navigate to a task and/or common instrument, never to an
-- account-specific holding. Financial commands still use holding IDs.
...
begin
  ...
  if exists(select 1 from jsonb_object_keys(input_payload) as field(name)
    where field.name not in ('title','body','occurred_at','timezone','task_id',
  ...
  insert into public.activity_events(user_id,source,action_type,target_table,after_data,status,
    occurred_at,occurrence_on,title,body,task_id,instrument_id,updated_at)
...
begin
  ...
  update public.activity_events set title=next_title,body=next_body,occurred_at=next_occurred_at,
    ...
    task_id=next_task,holding_id=null,account_id=null,instrument_id=next_instrument,
