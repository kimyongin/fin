-- Activity-only sharing must not expose the title/content of a private task
-- through the task's completion event. Timeline and search already use this
-- same boundary; align direct detail reads with them.
...
begin
    ...
    task_allowed := selected_owner = auth.uid() or public.can_view_feature(selected_owner, 'tasks');
    ...
    return (
        ...
        where event.id = input_activity_id
          ...
          and (task_allowed or event.task_id is null)
