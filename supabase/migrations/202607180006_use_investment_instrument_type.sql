...
set instrument_type = 'investment'
where instrument_type not in ('investment', 'cash', 'fx')
   or instrument_type is null;

alter table public.instruments
add constraint instruments_instrument_type_check
check (instrument_type in ('investment', 'cash', 'fx'));
...
begin
    ...
    loop
        ...
        normalized_instrument_type := lower(trim(coalesce(input_row ->> 'instrument_type', 'investment')));

        if normalized_account_name = '' then raise exception 'Account name is required'; end if;
        if normalized_ticker = '' then raise exception 'Ticker is required'; end if;
        if normalized_display_name = '' then raise exception 'Display name is required'; end if;
        if normalized_currency not in ('KRW', 'USD', 'JPY') then raise exception 'Currency must be KRW, USD, or JPY'; end if;
        if normalized_instrument_type not in ('investment', 'cash') then raise exception 'Invalid user-editable instrument type'; end if;
        if coalesce((input_row ->> 'quantity')::numeric, -1) < 0 then raise exception 'Quantity must be zero or greater'; end if;
        if coalesce((input_row ->> 'avg_price')::numeric, -1) < 0 then raise exception 'Average price must be zero or greater'; end if;

        select a.id into resolved_account_id
        from public.accounts a
        where a.user_id = current_user_id and lower(a.name) = lower(normalized_account_name)
        order by a.id limit 1;

        if resolved_account_id is null then
            insert into public.accounts (user_id, name, broker, is_active)
            values (current_user_id, normalized_account_name, nullif(trim(coalesce(input_row ->> 'broker', '')), ''), true)
            returning id into resolved_account_id;
            created_account_count := created_account_count + 1;
        else
            update public.accounts
            set broker = nullif(trim(coalesce(input_row ->> 'broker', '')), ''), is_active = true
            where id = resolved_account_id and user_id = current_user_id;
        end if;
        ...
        ) then raise exception 'Tag not found'; end if;

        select i.id into resolved_instrument_id
        from public.instruments i where i.user_id = current_user_id and i.ticker = normalized_ticker;
        if resolved_instrument_id is null then
            ...
            update public.instruments
            set display_name = normalized_display_name, currency = normalized_currency, instrument_type = normalized_instrument_type
            where id = resolved_instrument_id and user_id = current_user_id;
        end if;

        delete from public.instrument_tags where user_id = current_user_id and ticker = normalized_ticker;
        ...
        where h.user_id = current_user_id and h.account_id = resolved_account_id and h.ticker = normalized_ticker;
        if existing_holding <> '{}'::jsonb then before_rows := before_rows || jsonb_build_array(existing_holding); end if;

        insert into public.holdings (user_id, account_id, ticker, quantity, avg_price, note)
        values (current_user_id, resolved_account_id, normalized_ticker, (input_row ->> 'quantity')::numeric, (input_row ->> 'avg_price')::numeric, nullif(trim(coalesce(input_row ->> 'note', '')), ''))
        on conflict on constraint holdings_account_id_ticker_key do update
        set quantity = excluded.quantity, avg_price = excluded.avg_price, note = excluded.note
        returning id into saved_holding_id;

        select to_jsonb(h) into existing_holding from public.holdings h
    ...
    values (current_user_id, 'user', 'bulk_edit_portfolio', 'portfolio', null, jsonb_build_object('holdings', before_rows), jsonb_build_object('holdings', after_rows, 'row_count', saved_holding_count), 'succeeded')
