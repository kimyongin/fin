-- Add a first-class activity-kind filter while preserving the existing search and paging contract.
-- Drop the old signature to avoid ambiguous default-argument overloads in PostgREST.
drop function public.app_search_activities(uuid,text,date,date,text,boolean,bigint,bigint,uuid[],text,integer,jsonb,text);
...
begin
    ...
    return (with task_rows as (
      ...
      where tasks_allowed and normalized_kind in ('all','task') and normalized_state in ('all','todo') and task.user_id=selected_owner and task.control_state='active'
      ...
      where activity_allowed and normalized_kind <> 'task' and normalized_state in ('all','done') and event.user_id=selected_owner and event.status='succeeded'
        and (selected_owner=auth.uid() or event.record_kind not in ('research','review','decision','retrospective','trade','reconciliation'))
        and (normalized_kind = 'all' or coalesce(event.record_kind,'general') = normalized_kind)
...
revoke all on function public.app_search_activities(uuid,text,date,date,text,boolean,bigint,bigint,uuid[],text,integer,jsonb,text,text) from public, anon;
grant execute on function public.app_search_activities(uuid,text,date,date,text,boolean,bigint,bigint,uuid[],text,integer,jsonb,text,text) to authenticated;
