-- During the transition, preserve the old context shape and add the new
-- owner-only principle source for analysis. The old policy key is retired later.
alter function public.app_create_daily_context(text, text[])
    rename to app_create_daily_context_before_principles;
...
begin
    ...
    created := public.app_create_daily_context_before_principles(input_timezone, input_subject_tickers);
    update public.daily_review_contexts context
    set snapshot=jsonb_set(context.snapshot, '{principles}',
        jsonb_build_object('status','available', 'items',
            public.app_list_principles(null,input_timezone,false)->'items'))
