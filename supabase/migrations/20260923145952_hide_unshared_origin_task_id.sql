-- An activity grant alone does not reveal the identifier of a private task.
...
begin
    ...
    return (
        select jsonb_build_object(
            ...
            'task_id', case when task_allowed then event.task_id else null end,
            ...
            'origin_task', case when not task_allowed or event.task_id is null then null else (
                select jsonb_build_object(
                    'id', task.id,
                    'title', task.title,
                    'kind', task.kind,
