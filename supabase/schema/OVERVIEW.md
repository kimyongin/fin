...
| `app_list_todo_bundles`, `app_get_todo_bundle`, `app_save_todo_bundle` | Page owner-only work bundles, read ordered items and typed relations, or atomically upsert supplied items while preserving omissions and explicitly removing named item IDs. Bundle status is derived from current item/task state rather than copied. |
