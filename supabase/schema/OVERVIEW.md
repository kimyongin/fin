...
| `app_preview_holding_reconciliation`, `app_reconcile_holding`, `app_verify_holding`, `app_get_holding_integrity` | Preview/apply an absolute market, valuation, or cash checkpoint; record only explicitly compared brokerage fields; and read whether the latest verification predates a holding change. Price sync and briefing writes do not affect verification. An identical verification retry returns its stored success before checking the now-current holding version. |
