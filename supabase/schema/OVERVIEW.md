...
| Holding theses | `holding_theses`, `holding_thesis_history`, `holding_thesis_mutation_receipts` | Owner-only current reasons for holding an instrument, with an optional account override, horizon, review condition/date, expected-version history, and idempotent writes. A fully closed account position deactivates its override; zero total holdings deactivates the instrument thesis. Repurchase never silently reactivates an old thesis. |
