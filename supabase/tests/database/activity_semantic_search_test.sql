...
select public.app_create_activity('21111111-1111-4111-8111-111111111111',jsonb_build_object('title','손실 위험 줄이기','result','비중 축소 검토','timezone','Asia/Seoul','authored_via','app'));
select public.app_create_activity('21222222-2222-4222-8222-222222222222',jsonb_build_object('title','배당 유지 확인','result','현금흐름 유지','timezone','Asia/Seoul','authored_via','app'));
...
select extensions.is(
  public.app_search_activity_semantic(
    ('['||array_to_string(array[1.0::real]||array_fill(0.0::real,array[383]),',')||']')::extensions.vector,
    null,null,null,null,null,null,null,'all',10,'Asia/Seoul'
  )#>>'{items,0,title}',
  '손실 위험 줄이기',
...
select public.app_update_activity((select id from activity_events where title='손실 위험 줄이기'),1,
  '21333333-3333-4333-8333-333333333333','{"result":"위험 해소"}'::jsonb,'app');
...
select extensions.is(jsonb_array_length(public.app_search_activity_semantic(
    ('['||array_to_string(array_fill(0.0::real,array[383])||array[1.0::real],',')||']')::extensions.vector,
    null,null,null,true,null,null,null,'all',10,'Asia/Seoul'
