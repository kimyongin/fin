...
select extensions.lives_ok($$select public.app_link_trade_to_task((select id from trade_entries order by created_at desc limit 1),(select id from portfolio_tasks where title='10주 매수'),1,'33333333-3333-4333-8333-333333333333','agent')$$,'links first fill');
...
select extensions.lives_ok($$select public.app_link_trade_to_task((select id from trade_entries order by created_at desc limit 1),(select id from portfolio_tasks where title='10주 매수'),1,'55555555-5555-4555-8555-555555555555','agent')$$,'links second fill');
...
select extensions.throws_ok($$select public.app_link_trade_to_task((select id from trade_entries order by created_at desc limit 1),(select id from portfolio_tasks where title='10주 매수'),1,'66666666-6666-4666-8666-666666666666','agent')$$,'23505',null,'one fill cannot be linked twice');
...
select extensions.lives_ok($$select public.app_reverse_trade_entry((public.app_preview_trade_reversal((select id from trade_entries order by created_at desc limit 1),'잘못 기록한 두 번째 체결')->>'preview_id')::uuid,'88888888-8888-4888-8888-888888888888','agent')$$,'reverses a linked fill');
