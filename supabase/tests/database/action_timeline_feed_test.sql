...
select extensions.is((select count(*)::integer from jsonb_array_elements(public.app_list_action_timeline(null,'all',null,null,30,null,'Asia/Seoul')->'days') day, jsonb_array_elements(day->'items') item where item->>'title'='완료할 일반 과제'),1,'task completion has one representative record');
