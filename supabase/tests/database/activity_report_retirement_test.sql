...
select public.app_create_activity('91000000-0000-4000-8000-000000000003','{"title":"판단 활동","category":"decision","context":{"decision_state":"adopted","selected_option":"유지","reason":"기준 충족"}}'::jsonb);
select extensions.is(public.app_get_activity_report_context((clock_timestamp() at time zone 'Asia/Seoul')::date,(clock_timestamp() at time zone 'Asia/Seoul')::date,'Asia/Seoul',200,null)#>>'{decisions,0,question}','판단 활동','period decisions come from activity events');
