...
insert into body_api_record select public.app_create_activity_with_tags(
  ...
  '{"title":"분기 점검","body":"확인한 사실과 출처\n\n다음 행동은 보류","holding_id":9981,"authored_via":"app"}'::jsonb,
  array[]::uuid[]);
select extensions.is((select response->>'body' from body_api_record),E'확인한 사실과 출처\n\n다음 행동은 보류','new activity stores one body');
select extensions.is((select (response->>'holding_id')::bigint from body_api_record),9981::bigint,'holding reference is returned');
select extensions.is((select (response->>'account_id')::bigint from body_api_record),9981::bigint,'holding account is resolved by the server');
...
select extensions.throws_ok($$select public.app_create_activity('00000000-0000-0000-0000-000000009983',
  '{"title":"bad reference","holding_id":999999}'::jsonb)$$,'P0001','Holding reference not found','invalid holding link is rejected');
