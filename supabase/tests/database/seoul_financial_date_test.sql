...
select ok(position('Asia/Seoul' in pg_get_functiondef('public.app_preview_holding_reconciliation(bigint,jsonb,text,date)'::regprocedure)) > 0, 'holding correction preview validates the Seoul business date');
