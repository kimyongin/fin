...
select extensions.is(jsonb_array_length(public.app_get_activity_report_context(current_date,current_date,'Asia/Seoul',200,null)->'events'),1,'context returns period source events');
select extensions.is(public.app_get_activity_report_context(current_date,current_date,'Asia/Seoul',200,null)->>'current_open_tasks_basis','current_at_request_not_historical_period_end','context labels current pending snapshot honestly');
...
create temporary table saved_report as select public.app_save_activity_report(null,null,'19222222-2222-4222-8222-222222222222',jsonb_build_object(
  'period_kind','daily','period_start',current_date,'period_end',current_date,'timezone','Asia/Seoul','title','오늘 행동 리포트','summary','한 가지 행동을 확인했다.',
