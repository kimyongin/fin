...
select public.app_create_activity('97111111-1111-4111-8111-111111111111',
  '{"title":"정기 점검","body":"확인한 공개 사실","holding_id":9971}'::jsonb);
...
select extensions.is(public.app_get_activity((select id from activity_target),
  '00000000-0000-0000-0000-000000001971')->>'holding_id',null::text,'asset reference is hidden without asset grant');
select extensions.is(public.app_get_activity((select id from activity_target),
  '00000000-0000-0000-0000-000000001971')->>'holding_summary',null::text,'asset summary is hidden without asset grant');
select extensions.is(public.app_get_activity((select id from activity_target),
...
select extensions.throws_ok($$select public.app_search_activities(input_owner_user_id=>'00000000-0000-0000-0000-000000001971',input_holding_id=>9971)$$,
