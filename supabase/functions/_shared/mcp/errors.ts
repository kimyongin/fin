...
export function classifyPortfolioError(error: unknown): PortfolioErrorInfo {
  ...
  if (message.includes('version conflict') || message.includes('note conflict')) return result('version_conflict', 'The record changed after it was read.', true, 'Re-read the record and retry with its current version or note.')
