...
const guideSources: Record<WorkflowGuideTopic, WorkflowGuideSource> = {
  ...
  todo: {
    ...
    steps: [
      ...
      { id: 'save-future-task', title: 'Save future intent', instruction: 'Call save_general_task for one user-meaningful future follow-up. On standalone creation, optional existing tag_ids are saved in the same transaction; retry an uncertain result with the same key, content, and tags. When it directly follows a recorded activity, pass origin_activity_id. Use daily recurrence only for an explicitly recurring task.', tools: ['save_general_task'] },
      ...
      { id: 'record-unplanned-work', title: 'Record completed work', instruction: 'Call record_manual_activity for explicit user-reported work already done when there is no matching task to complete. Optional existing tag_ids are saved in the same transaction; retry an uncertain result with the same key, content, and tags. Default to general; classify research, review, decision, or retrospective only when it describes that work. Preserve factual source URLs and scope in context when supplied, separate facts from the conclusion, and do not infer a trade, balance correction, or task completion from the category. Result and conclusion are optional.', tools: ['record_manual_activity'] },
      ...
      { id: 'find-and-tag', title: 'Find or classify activities', instruction: 'Use search_activities for combined keyword, date, state, conclusion, account, instrument, and activity-tag filters. Optional record_kinds selects multiple activity kinds with OR; different filter dimensions still use AND. An empty kind list means all kinds. Kinds are built-in classifications, distinct from user-created tags. Kind-filtered searches use keyword results only; otherwise semantic similarity can supplement the first completed-activity page. Inspect semantic_status and keep valid keyword results when unavailable. Shared review results contain only permitted public summary fields, never private notes, tags or source context. Read list_activity_tags before setting known tag IDs. Tags classify records only and never affect allocation.', tools: ['search_activities', 'list_activity_tags', 'set_activity_tags', 'set_general_task_tags'] },
