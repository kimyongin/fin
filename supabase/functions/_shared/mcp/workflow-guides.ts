...
const guideSources: Record<WorkflowGuideTopic, WorkflowGuideSource> = {
  ...
  holding_thesis: {
    ...
    purpose: 'Clarify why an instrument is held and save one approved common instrument note.',
    scenario_ids: ['W02', 'W04'],
    related_tools: ['get_portfolio_state', 'find_holdings', 'update_entity_note', 'save_general_task'],
    source_paths: [
      'supabase/functions/_shared/mcp/portfolio-tools.ts',
      'supabase/functions/portfolio-mcp-oauth/index.ts',
      'supabase/migrations/20260922040431_entity_notes_and_verification_notes.sql',
      'supabase/migrations/20260924152000_decouple_asset_detail_from_legacy_notes.sql',
    ],
    steps: [
      { id: 'resolve-holding', title: 'Resolve the instrument', instruction: 'Use get_portfolio_state and find_holdings when the instrument is ambiguous. The note belongs to the instrument, not to an account holding.', tools: ['get_portfolio_state', 'find_holdings'] },
      { id: 'read-current', title: 'Read the saved note', instruction: 'Read the instrument note from get_portfolio_state. Missing means unknown, not an inferred reason.', tools: ['get_portfolio_state'] },
    ...
    recovery: [
      ...
      'For an ambiguous instrument, stop before writing and ask the user to select it.',
      'For a lost write response, read the instrument note first; if the requested text is saved, report success without another write.',
    ],
    unavailable_steps: [],
  },
  daily_review: {
  ...
  trade_entry: {
    ...
    steps: [
      ...
      { id: 'resolve-input', title: 'Resolve the trade fields', instruction: 'Identify account, instrument, side, quantity, unit price, and trade date. A registered instrument with no holding is not proof of an owned position. Use find_holdings and ask the user when multiple accounts or instruments match. If date is omitted, state the user-timezone date you will use.', tools: ['find_holdings'] },
  ...
  reconciliation: {
    ...
    purpose: 'Correct an incorrect current holding with an absolute value. An incorrect local trade is corrected from the actual brokerage values, not by replaying old trades. A comparison that changes nothing needs no saved verification state.',
    scenario_ids: ['W06', 'W08', 'S18', 'S19', 'S20', 'S22'],
    related_tools: ['list_principles', 'find_holdings', 'preview_holding_reconciliation', 'reconcile_holding', 'save_general_task'],
    source_paths: [
      'supabase/functions/_shared/mcp/portfolio-tools.ts',
      'supabase/functions/portfolio-mcp-oauth/index.ts',
      'supabase/migrations/20260925122847_retire_holding_verification.sql',
    ...
    steps: [
      ...
      { id: 'classify-operation', title: 'Decide whether values need correction', instruction: 'Use reconciliation only when actual current values differ from the local holding, including after an incorrect local trade entry. If values already match, no financial write is needed.', tools: [] },
      { id: 'resolve-and-read', title: 'Resolve and inspect the holding', instruction: 'Use find_holdings to read the current holding and version. Ask the user to confirm current brokerage quantity and average cost before correcting a wrong trade entry.', tools: ['find_holdings'] },
      { id: 'preview-correction', title: 'Estimate an absolute correction', instruction: 'Call preview_holding_reconciliation with actual values. It saves nothing; explain the effect and retain the returned holding version. This is not a trade.', tools: ['preview_holding_reconciliation'] },
      { id: 'commit-correction', title: 'Apply the confirmed correction', instruction: 'After the user confirms the estimate, call reconcile_holding with the exact values, reason, date, expected holding version, and an idempotency key. A version conflict requires re-reading and a fresh estimate.', tools: ['reconcile_holding'] },
      { id: 'save-followup-if-requested', title: 'Optionally save a future follow-up', instruction: 'A successful correction already creates an automatic activity; do not restate it as a completed task. If the user asks to remember a separate future follow-up, save one general task.', tools: ['save_general_task'] },
    ],
    boundaries: [
      ...
      'No verified, stale-verification, or unchecked holding status is maintained. A comparison without a value change needs no financial write.',
  ...
  todo: {
    ...
    related_tools: ['list_general_tasks', 'list_due_general_tasks', 'get_general_task', 'get_activity', 'search_activities', 'list_activity_tags', 'save_general_task', 'delete_general_task', 'transition_general_task', 'record_manual_activity', 'update_activity', 'delete_activity', 'set_activity_tags', 'set_general_task_tags'],
    source_paths: [
      ...
      'supabase/migrations/20260924173000_activity_record_date_filter.sql',
      'supabase/migrations/20260924175000_agent_due_recurrence.sql',
      'supabase/migrations/20260926132021_task_delete_and_atomic_activity_tags.sql',
    ],
    steps: [
      ...
      { id: 'read-existing', title: 'Read existing and due tasks', instruction: 'Use list_due_general_tasks when asked to check and proceed with due work; follow next_offset until null and read returned tasks before acting. Use list_general_tasks and get_general_task for all future follow-ups. A due read does not run work. Research results and decisions are activities, not a second kind of task.', tools: ['list_due_general_tasks', 'list_general_tasks', 'get_general_task'] },
      { id: 'save-future-task', title: 'Save future intent', instruction: 'Call save_general_task for one user-meaningful future task. Set none/daily/weekly recurrence only when requested; weekly uses ISO weekdays 1=Monday through 7=Sunday and optional local HH:MM in the task timezone. On edits, omit tag_ids to preserve tags or pass [] to clear them; fields and specified tags save together. Retry an uncertain result with the same key and exact inputs. Tasks are independent of recorded activities.', tools: ['save_general_task'] },
      { id: 'complete-existing', title: 'Perform, record, or end the matching task', instruction: 'For a request to check and proceed, first call list_due_general_tasks. Carry out only work within the user’s authority; a trade or external action may require explicit approval. After real success, call transition_general_task with complete and the returned occurrence_on and expected_version. The server writes one linked completion activity with the actual result. On failure, missing evidence, or needed user input, leave the task unfinished and report why. Reopen corrects an erroneous completion; cancel ends future repeats without inventing performance. Never create another manual activity for the same completion.', tools: ['list_due_general_tasks', 'transition_general_task'] },
      ...
      { id: 'revise-current-activity', title: 'Revise the same activity', instruction: 'Read get_activity, then call update_activity when the user corrects a title, Markdown body, date, related reference or tags. Omit tag_ids to preserve tags; [] clears them. Fields and provided tags save together. This applies to automatic records too but never changes the underlying holding or task state.', tools: ['get_activity', 'update_activity'] },
      ...
      { id: 'delete-task', title: 'Remove an unwanted task', instruction: 'Only after explicit confirmation, read get_general_task and call delete_general_task with its version. This removes future intent and detaches existing records; it does not delete past records or undo a completed action. To keep a repeating definition but end future repetitions, use transition_general_task action cancel instead.', tools: ['get_general_task', 'delete_general_task'] },
    ...
    boundaries: [
      ...
      'Whole-portfolio sharing exposes readable activity titles, bodies, tags, linked task summaries, and instrument context together to connected readers. Internal execution payloads remain private.',
  ...
  activity_report: {
    ...
    boundaries: [
      ...
      'Saving a retrospective adds one editable activity. It is readable under whole-portfolio sharing but does not mutate tasks, trades, or holdings.',
  ...
  product_feedback: {
    ...
    related_tools: ['submit_product_feedback', 'list_my_product_feedback', 'get_my_product_feedback', 'update_my_product_feedback', 'delete_my_product_feedback', 'list_product_feedback_admin', 'update_product_feedback_admin'],
    source_paths: [
      'docs/design/product-feedback.md',
      'supabase/functions/_shared/mcp/portfolio-tools.ts',
      'supabase/functions/portfolio-mcp-oauth/index.ts',
      'supabase/migrations/202609210028_product_feedback.sql',
      'supabase/migrations/20260926143300_feedback_owner_crud.sql',
    ...
    steps: [
      ...
      { id: 'verify-or-review', title: 'Verify and review status', instruction: 'Treat the returned feedback ID and status as the save result. Use list_my_product_feedback for a later status question, then get_my_product_feedback for the exact current version before a correction or removal. A read-back failure after successful submission does not mean the submission failed.', tools: ['list_my_product_feedback','get_my_product_feedback'] },
      { id: 'correct-or-remove', title: 'Correct or remove my item', instruction: 'On explicit request, update only the reporter body or delete the exact item at its current version. If deleting an item linked to a GitHub issue, explain that the external issue remains. Version conflicts require a fresh read.', tools: ['get_my_product_feedback','update_my_product_feedback','delete_my_product_feedback'] },
      { id: 'administrator-triage', title: 'Administrator triage', instruction: 'Only for an authenticated allowlisted feedback administrator, list the triage queue and update its status, public response or GitHub issue URL with the current version. These tools do not manage allowlist membership or external GitHub issues.', tools: ['list_product_feedback_admin','update_product_feedback_admin'] },
    ],
    boundaries: [
