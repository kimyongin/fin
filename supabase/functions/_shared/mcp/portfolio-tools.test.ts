...
describe('portfolio MCP tool definitions', () => {
  ...
  it('separates planned general tasks from reported completed activity', () => {
    ...
    expect(tool('save_general_task').description).toContain('Fields and provided tags save atomically')
    expect(tool('update_activity').description).toContain('fields and tags save atomically')
