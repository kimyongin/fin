function EntityTitle({ as: Tag = 'h2', children }) {
  return <Tag className="type-item-title text-[var(--ink)]">{children}</Tag>
}
...
export function AccountIdentity({ account, as, detail }) {
  ...
  return (
    <div className="min-w-0">
      ...
      {description && <p className="type-secondary mt-1 truncate text-[var(--muted-ink)]">{description}</p>}
      {account.note && <p className="type-secondary mt-2 line-clamp-2 text-[var(--muted-ink)]">{account.note}</p>}
    </div>
  )
}
...
export function InstrumentIdentity({ instrument, as, detail }) {
  ...
  return (
    <div className="min-w-0">
      ...
      {description && <p className="type-secondary mt-1 truncate text-[var(--muted-ink)]">{description}</p>}
      {instrument.note && <p className="type-secondary mt-2 line-clamp-2 text-[var(--muted-ink)]">{instrument.note}</p>}
    </div>
  )
}
