...
function StrategyDashboard({ canEdit, onEdit, strategyState, tagCards, totalValue }) {
  ...
  return (
    <section className="grid gap-5">
      ...
      <article className="overflow-hidden rounded-lg border border-[var(--line)] bg-[var(--panel)] shadow-[var(--shadow-soft)]"><div className="border-b border-[var(--line)] px-5 py-4"><h2 className="text-lg font-semibold">목표 대비 현재 비중</h2></div><div className="divide-y divide-[var(--line)]">{rows.map((row) => { const targetWidth = Math.max(0, Math.min(row.targetPercentage, 100)); const currentWidth = Math.max(0, Math.min(row.currentPercentage, 100)); return <div className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(9rem,1fr)_7rem_7rem_8rem] sm:items-center" key={row.id}><div><div className="flex items-baseline justify-between gap-3"><p className="truncate text-sm font-semibold">{row.name}</p><span className="text-xs text-[var(--muted-ink)]">{formatKrw(row.value)}</span></div><div className="relative mt-2 h-2 overflow-hidden rounded-full bg-[var(--surface-3)]"><span className="absolute inset-y-0 left-0 bg-[var(--accent-soft)]" style={{ width: `${targetWidth}%` }} /><span className="absolute left-0 top-0 h-full rounded-full bg-[var(--accent)]" style={{ width: `${currentWidth}%` }} /></div></div><span className="text-sm text-[var(--muted-ink)]">목표 {formatPercent(row.targetPercentage)}</span><span className="text-sm">현재 {formatPercent(row.currentPercentage)}</span><span className={`text-sm font-semibold ${row.differencePercentage < 0 ? 'text-emerald-300' : row.differencePercentage > 0 ? 'text-amber-300' : 'text-[var(--muted-ink)]'}`}>{row.differencePercentage > 0 ? '부족 ' : row.differencePercentage < 0 ? '초과 ' : ''}{formatPercent(Math.abs(row.differencePercentage))}p</span></div> })}</div></article>
