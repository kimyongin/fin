...
import { createEmptyNewsState, deleteNewsFact, deleteNewsFactAnnotation, fetchNewsState, saveNewsFact, saveNewsFactAnnotation, updateNewsFact } from './data'
...
const recordTitle = (body) => body.trim().split('\n').find(Boolean)?.trim().slice(0, 100) || '뉴스 팩트'
...
function FactModal({ defaultCountryCode, error, onClose, onSave, saving }) {
  const [body, setBody] = useState('')
  const [countryCode, setCountryCode] = useState(defaultCountryCode)
  const [opinion, setOpinion] = useState('')
  const dirty = body !== '' || countryCode !== defaultCountryCode || opinion !== ''
  const actions = (requestClose) => (
    <ModalActions
      disabled={saving}
      onClose={requestClose}
      onSave={() => onSave({ body, countryCode, opinion })}
  ...
  return (
    <ModalShell closeDisabled={saving} dirty={dirty} footer={actions} onClose={onClose} title="뉴스 팩트 기록">
      <div className="grid gap-4">
        ...
        <label className="grid gap-2">
          ...
          <textarea className="min-h-24 w-full min-w-0 rounded-2xl border border-[var(--line)] bg-[var(--surface-3)] px-3 py-3 text-sm leading-6 outline-none transition focus:border-[var(--accent)]" onChange={(event) => setOpinion(event.target.value)} placeholder="이 팩트가 시장과 내 태그에 주는 의미, 감지한 신호를 기록합니다." value={opinion} />
        </label>
        {error && <p className="rounded-2xl border border-red-400/40 bg-red-500/10 px-3 py-2.5 text-sm text-red-100">{error}</p>}
      </div>
    </ModalShell>
  )
}
...
export default function NewsPage({ canEdit, ownerUserId = null, supabase }) {
  const [state, setState] = useState(createEmptyNewsState()); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [showFactModal, setShowFactModal] = useState(false); const [editingFact, setEditingFact] = useState(null); const [saving, setSaving] = useState(false); const [countryFilter, setCountryFilter] = useState('all')
  ...
  const saveFact = async ({ body, countryCode, opinion }) => { if (await run(async () => { const fact = await saveNewsFact(supabase, { fact_date: today(), country_code: countryCode, axis: 'general', title: recordTitle(body), source_name: '', source_url: '', body }); if (opinion.trim()) await saveNewsFactAnnotation(supabase, { fact_id: fact.id, signal: 'observe', body: opinion }) }, '기록을 저장하지 못했습니다.')) setShowFactModal(false) }
  ...
  return <section className="grid gap-5"><div className="flex flex-wrap items-center justify-between gap-3"><select aria-label="뉴스 국가 필터" className="min-h-11 min-w-28 rounded-xl border border-[var(--line)] bg-[rgba(255,255,255,0.03)] px-3 text-sm text-[var(--ink)] outline-none transition focus:border-[var(--accent)]" onChange={(event) => setCountryFilter(event.target.value)} value={countryFilter}><option value="all">전체</option><option value="KR">한국</option><option value="US">미국</option></select>{canEdit && <button className="min-h-11 rounded-xl bg-[var(--accent)] px-4 text-sm font-semibold text-white transition hover:brightness-95" onClick={() => setShowFactModal(true)} type="button">뉴스 추가</button>}</div>{error && !showFactModal && !editingFact && <p className="rounded-lg border border-red-400/40 bg-red-500/10 px-4 py-3 text-sm text-red-100">{error}</p>}{loading ? <p className="text-sm text-[var(--muted-ink)]">뉴스 기록을 불러오는 중입니다.</p> : groups.length === 0 ? <p className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] px-4 py-8 text-center text-sm text-[var(--muted-ink)]">{countryFilter === 'all' ? '뉴스 기록이 없습니다.' : `${countryLabel.get(countryFilter)} 뉴스 기록이 없습니다.`}</p> : <article className="rounded-[28px] border border-[var(--line)] bg-[var(--panel)] p-5 shadow-[var(--shadow-soft)]"><div className="grid gap-7">{groups.map((group) => <section key={group.label}><h2 className="border-b border-[var(--line)] pb-2 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--muted-ink)]">{group.label}</h2><ol className="relative mt-4 border-l border-[var(--line)] sm:border-l-0">{group.facts.map((fact) => <TimelineFact canEdit={canEdit} fact={fact} key={fact.id} onEdit={setEditingFact} />)}</ol></section>)}</div></article>}{showFactModal && <FactModal defaultCountryCode={countryFilter === 'all' ? 'KR' : countryFilter} error={error} onClose={() => { setShowFactModal(false); setError('') }} onSave={saveFact} saving={saving} />}{editingFact && <RecordEditor canEdit={canEdit} error={error} fact={state.facts.find((fact) => fact.id === editingFact.id) ?? editingFact} onClose={() => { setEditingFact(null); setError('') }} onDelete={deleteFact} onSave={saveEdit} saving={saving} />}</section>
}
