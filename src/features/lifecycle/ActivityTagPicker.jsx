...
export default function ActivityTagPicker({ compact = false, disabled = false, onChange, onTagsChanged, selectedIds = [], supabase, tags = [] }) {
  ...
  return <fieldset className="grid gap-2" disabled={disabled || busy}>
    ...
    {compact && <><div className="flex flex-wrap gap-2">{tags.filter((tag) => selectedIds.includes(tag.id)).map((tag) => <button aria-label={`${tag.name} 태그 해제`} className="min-h-11 rounded-full border border-[var(--accent)] px-3 text-sm" key={tag.id} onClick={() => toggle(tag.id)} type="button">{tag.name} ×</button>)}</div><input aria-label="태그 검색" className="min-h-11 w-full rounded-2xl border border-[var(--line)] bg-[var(--surface-3)] px-3 text-base" onChange={(event) => setQuery(event.target.value)} placeholder="태그 검색" value={query} />{query.trim() && <div className="max-h-48 overflow-y-auto rounded-xl border border-[var(--line)]">{tags.filter((tag) => !selectedIds.includes(tag.id) && tag.name.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())).slice(0, 20).map((tag) => <button className="block min-h-11 w-full px-3 text-left text-sm" key={tag.id} onClick={() => { toggle(tag.id); setQuery('') }} type="button">{tag.name}</button>)}</div>}</>}
    {!compact && tags.length > 0 && <div className="flex flex-wrap gap-2">{tags.map((tag) => <label className={`flex min-h-11 items-center gap-2 rounded-full border px-3 text-sm ${selectedIds.includes(tag.id) ? 'border-[var(--accent)] bg-[var(--accent)]/10' : 'border-[var(--line)]'}`} key={tag.id}><input checked={selectedIds.includes(tag.id)} onChange={() => toggle(tag.id)} type="checkbox" />{tag.name}</label>)}</div>}
