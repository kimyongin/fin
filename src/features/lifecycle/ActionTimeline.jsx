          ...
          <div className="grid gap-4 border-t border-[var(--line)] pt-4">
            <fieldset className="min-w-0">
              ...
              <div className="flex flex-wrap gap-2">
                <button aria-pressed={searchDraft.kinds.length === 0} className={`min-h-11 rounded-full border px-3 text-sm ${searchDraft.kinds.length === 0 ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]' : 'border-[var(--line)] text-[var(--muted-ink)]'}`} onClick={() => setSearchDraft({ ...searchDraft, kinds: [] })} type="button">전체</button>
                {activityKinds.map(({ id, label }) => <label className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-full border px-3 text-sm focus-within:ring-2 focus-within:ring-[var(--accent)] ${searchDraft.kinds.includes(id) ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]' : 'border-[var(--line)] text-[var(--muted-ink)]'}`} key={id}>
                  <input checked={searchDraft.kinds.includes(id)} className="accent-[var(--accent)]" onChange={() => setSearchDraft((current) => ({ ...current, kinds: current.kinds.includes(id) ? current.kinds.filter((value) => value !== id) : [...current.kinds, id] }))} type="checkbox" />{label}
                </label>)}
              </div>
