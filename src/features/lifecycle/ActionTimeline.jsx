  ...
  return <section className="grid gap-5">
    <header className="rounded-2xl border border-[var(--line)] bg-[var(--panel)] p-3 sm:p-4">
      <form className="grid gap-3" onSubmit={(event) => { event.preventDefault(); requestGate.current.invalidate(); setLoading(true); setSearchApplied({ ...searchDraft }) }}>
        ...
        <details>
          <summary className="min-h-11 cursor-pointer py-3 text-sm text-[var(--muted-ink)]">상세 필터{searchDraft.kinds.length > 0 ? ` · 종류 ${searchDraft.kinds.length}개` : ''}</summary>
          <div className="grid gap-4 border-t border-[var(--line)] pt-4">
            <fieldset className="min-w-0">
              <legend className="mb-2 text-sm font-medium">활동 종류</legend>
              <div className="flex flex-wrap gap-2">
                <button aria-pressed={searchDraft.kinds.length === 0} className={`min-h-11 rounded-full border px-4 text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)] ${searchDraft.kinds.length === 0 ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]' : 'border-[var(--line)] text-[var(--muted-ink)]'}`} onClick={() => setSearchDraft({ ...searchDraft, kinds: [] })} type="button">전체</button>
                {activityKinds.map(({ id, label }) => <button aria-pressed={searchDraft.kinds.includes(id)} className={`min-h-11 rounded-full border px-4 text-sm font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--accent)] ${searchDraft.kinds.includes(id) ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-[var(--accent)]' : 'border-[var(--line)] text-[var(--muted-ink)]'}`} key={id} onClick={() => setSearchDraft((current) => ({ ...current, kinds: current.kinds.includes(id) ? current.kinds.filter((value) => value !== id) : [...current.kinds, id] }))} type="button">{label}</button>)}
              </div>
              <p className="mt-2 text-xs text-[var(--muted-ink)]">여러 개를 선택하면 선택한 종류 중 하나에 해당하는 활동을 찾습니다.</p>
            </fieldset>
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <label className="grid gap-1 text-xs text-[var(--muted-ink)]">결론<select className="min-h-11 min-w-0 rounded-xl border border-[var(--line)] bg-[var(--surface-3)] px-3 py-2 text-sm text-[var(--ink)]" onChange={(event) => setSearchDraft({ ...searchDraft, conclusion: event.target.value })} value={searchDraft.conclusion}><option value="all">전체</option><option value="yes">결론 있음</option><option value="no">결론 없음</option></select></label>
            </div>
            {availableTags.length > 0 && <fieldset><legend className="mb-2 text-sm font-medium">태그</legend><div className="flex flex-wrap gap-2">{availableTags.map((tag) => <label className={`flex min-h-11 items-center gap-2 rounded-full border px-3 text-sm ${searchDraft.tagIds.includes(tag.id) ? 'border-[var(--accent)] bg-[var(--accent)]/10' : 'border-[var(--line)]'}`} key={tag.id}><input checked={searchDraft.tagIds.includes(tag.id)} onChange={() => setSearchDraft({ ...searchDraft, tagIds: searchDraft.tagIds.includes(tag.id) ? searchDraft.tagIds.filter((id) => id !== tag.id) : [...searchDraft.tagIds, tag.id] })} type="checkbox" />{tag.name}</label>)}</div></fieldset>}
          </div>
        </details>
      </form>
    </header>
