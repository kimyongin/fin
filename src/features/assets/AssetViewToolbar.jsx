...
export default function AssetViewToolbar({ canEdit, onCreateTag, onEditTag, tags = [] }) {
  const [managingTags, setManagingTags] = useState(false)
  if (!canEdit) return null
  return <>
    <div className="flex justify-end">
      <button className="min-h-11 rounded-xl border border-[var(--line)] px-4 text-sm font-semibold" onClick={() => setManagingTags(true)} type="button">태그 관리</button>
    </div>
