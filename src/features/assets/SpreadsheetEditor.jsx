...
function Cell({ changed, column, error, hasUndo, onChange, originalValue, row, tags }) {
  ...
  const className = `type-input h-11 w-full min-w-28 border-0 bg-transparent px-2.5 outline-none focus:bg-[var(--surface-2)] disabled:cursor-not-allowed disabled:bg-black/20 disabled:text-[var(--muted-ink)] ${hasUndo ? 'pr-12' : ''} ${error ? 'bg-red-500/10 text-red-100' : changed ? 'bg-red-500/10 text-red-300' : ''}`
  let control
...
function SpreadsheetTable({ errorsById, onChange, onPaste, onResetRow, rows, tags }) {
    ...
    <div className="relative overflow-hidden rounded-lg border border-[var(--line)] bg-[var(--panel)]">
      <div className="spreadsheet-scroll overflow-auto" onPaste={onPaste}>
        <table className="w-full min-w-[1080px] border-collapse text-left">
          <thead className="type-label bg-[var(--surface-2)] text-[var(--muted-ink)]"><tr>{spreadsheetColumns.map(([, label]) => <th className="border-b border-[var(--line)] px-2.5 py-3" key={label}>{label}</th>)}</tr></thead>
...
export default function SpreadsheetEditor({ accounts, canSave = true, csvCopied, holdings, instrumentTags, instruments, onClose, onCopyCsv, onDirtyChange, onSave, saving, tags }) {
  ...
  const footer = <div aria-label="표 편집 하단 작업" className="grid gap-3 sm:flex sm:flex-wrap sm:items-center sm:justify-between" role="group"><div className="grid gap-2 sm:flex sm:items-center sm:gap-3">{status}<div className="flex flex-wrap items-center gap-2"><button className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm font-medium text-[var(--muted-ink)] hover:bg-[var(--surface-2)]" onClick={onCopyCsv} title="저장된 자산 전체를 CSV로 복사" type="button">CSV 복사</button><span aria-live="polite" className="text-xs text-[var(--muted-ink)]" role="status">{csvCopied ? 'CSV를 복사했어요' : ''}</span></div></div>{actions}</div>
  const table = <SpreadsheetTable errorsById={errorsById} onChange={updateRow} onPaste={handlePaste} onResetRow={resetRow} rows={{ original: originalRows, visible: visibleRows }} tags={tags} />

  return (
    <ModalShell closeDisabled={saving} dirty={hasChanges} footer={footer} fullScreen onClose={onClose} title="표 편집">
      <div className="grid gap-4">
        <p className="text-sm leading-6 text-[var(--muted-ink)]">엑셀에서 복사한 행을 표 안에 붙여넣고 한 번에 저장할 수 있습니다.</p>
        {filters}
        {table}
